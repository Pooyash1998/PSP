...
ProcessID os_Scheduler_Random(Process const processes[], ProcessID current) {
	...
	uint8_t anzahl = 0;
	uint16_t RandMod ; 
	//berechen die Anzahl von wartenden Prozessen im Zustand Ready (ausser Idle)
...
ProcessID os_Scheduler_RoundRobin(Process const processes[], ProcessID current) {
   ...
   if(anzahl > 1) {
	   if (schedulingInfo.timeSlice == 1) //proc soll priority-mal durchgefuehrt werden. daher runterzaehlen bis 1 und nicht 0
	   {
		  ...
		  schedulingInfo.timeSlice = os_getProcessSlot(next)->priority;
		  return next;
	   }
	   else {
...
ProcessID os_Scheduler_InactiveAging(Process const processes[], ProcessID current) {
	...
	if (anzahl > 1)
	{
		...
		schedulingInfo.age[pidNextProc] = 0; //to zero or to its priority ?
		return pidNextProc;
	}
	return 0;
}
...
ProcessID os_Scheduler_RunToCompletion(Process const processes[], ProcessID current) {
    // This is a presence task
	 if (processes[current].state == OS_PS_READY ) 
		return current; //in diesem Versuch wird er nie terminiert.
	else 
