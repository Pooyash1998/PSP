...
/*Allocates a chunk of memory on the medium given by the driver.
This funktion will allocate memory for the given owner. 
...
{
	//Shared Memory should not be freed!
	if(os_getMapEntry(heap,os_getFirstByteOfChunk(heap,addr)) < SH_COMMANDS)
		os_freeOwnerRestricted(heap,addr,os_getCurrentProc());
	else
		os_error("Error:use os_shfree!");	
}
...
A pointer to the first Byte of the allocated chunk.
0 if allocation fails (0 is never a valid address).
#####Shared Memory Defnines for the First Bit: 
 * 0xE: Closed, 
 * 0xD: WriteOpen, 
 * 0xC: ReadOpen (2 readers),
 * 0xB: ReadOpen (1 reader);
*/
MemAddr os_sh_malloc(Heap *heap, size_t size)
{
	...
	// assign the Corresponding Nibbles to the procesID
	setMapEntry(heap, addr, SH_CLOSED);
	// set other Nibbles depending on size
	for(size_t i = 1 ; i< size ; i++)
	{
...
{
	MemAddr s = os_getFirstByteOfChunk(heap,os_sh_readOpen(heap, ptr)); 
	if(s != os_getUseStart(heap)+os_getUseSize(heap))
	{
		if(os_getChunkSize(heap, s) < offset + length ) 
			os_error("Fehler shmem zu klein!");
		else{
			s += offset;
			for(uint16_t i = 0; i < length; i++){
				intHeap->driver->write(((MemAddr)dataDest + i), heap->driver->read(s + i));
			}
		}
		os_sh_close(heap, s);
	}
...
MemAddr os_sh_readOpen(Heap const *heap, MemAddr const *ptr)
{
	...
	uint8_t val = os_getMapEntry(heap, addr);
	if(val != 0){
		if(val < SH_COMMANDS)
		{
			os_error("Can't read private memory!");
			os_leaveCriticalSection();
			return 0;
		}
		while(val == SH_WRITE || val == SH_READ2){
			os_yield();
			addr = os_getFirstByteOfChunk(heap,*ptr);
			val = os_getMapEntry(heap, addr);
			if(val == os_getCurrentProc()) 
			os_error("Can't read private memory!");
		}
		
		if(val == SH_READ1) 
			setMapEntry(heap, addr, SH_READ2);
		else
			setMapEntry(heap, addr, SH_READ1);	
		...
		os_leaveCriticalSection();
		return os_getUseSize(heap)+os_getUseStart(heap);
	}
}
/*
...
{
	MemAddr s = os_getFirstByteOfChunk(heap,os_sh_writeOpen(heap, ptr));
	if(s != (os_getUseStart(heap)+os_getUseSize(heap)))
	{
		if(os_getChunkSize(heap, s) < offset + length) 
			os_error("Fehler shmem zu klein!");
			...
			s += offset;
			for(uint16_t i = 0; i < length; i++){
				heap->driver->write(s + i, intHeap->driver->read((MemAddr)dataSrc + i));
			}
		}
		os_sh_close(heap, s);
	}
...
MemAddr os_sh_writeOpen(Heap const *heap, MemAddr const *ptr)
{
	...
	if(val != 0){
		if(val < SH_COMMANDS){
			os_error("Can't write in private memory!");
			os_leaveCriticalSection();
			return 0;
		}
		while(val != SH_CLOSED){
		...
		setMapEntry(heap, addr, SH_WRITE);
		addr = *ptr;
		os_leaveCriticalSection();
		return addr;
	}
...
void os_sh_free(Heap *heap, MemAddr *addr)
{
	...
	if(os_getMapEntry(heap,ad) >= SH_COMMANDS){
		while(os_getMapEntry(heap, ad) != SH_CLOSED)
		{
			os_yield();
			//check for address changes
			ad = os_getFirstByteOfChunk(heap,*addr);
		}
		uint16_t size = os_getChunkSize(heap,ad);
		for(uint16_t i=0; i<size; i++)
		{
			if(ad + i == os_getUseStart(heap)+os_getUseSize(heap)) break;
			setMapEntry(heap,ad + i, 0x0);
		}
	}
	else if(os_getMapEntry(heap, ad) != 0)
		os_error("Can't free private memory!");
...
void os_sh_close(Heap const *heap, MemAddr addr)
{
	...
	if(val != 0)
	{
		if(val < SH_COMMANDS)
		{
			os_error("Can't close private memory!");
		}
		else if(val == SH_READ2) 
			setMapEntry(heap, addr, SH_READ1);
		else
			setMapEntry(heap, addr, SH_CLOSED);
	}
