...
void os_init(void) {
	...
	//checking if the safty Heapoffset is big enough
	if((uint16_t)&__heap_start - AVR_SRAM_START > HEAPOFFSET)
		os_error("Error:bss overflow into Heap!");
	//initializing the internal and External SRAM
	initMemoryDevices();
...
void os_errorPStr(char const* str) {
	 ...
	 SREG &= ~(1 << 7); //MSB bzw Global Interrupt flag deaktivieren
	 lcd_clear();
	 lcd_writeErrorProgString(str);
	 while(os_getInput() != 0b00001001 ) //warte auf Enter + ESC (btn 1 und 4)
	 {
		 //warten auf User 
	 }
	 os_waitForNoInput(); //warten aufs loslassen der Tasten
	 
	 SREG |= gieb; //MSB bzw Global Interrupt flag auf den ursprunglichen Wert setzen.
	 lcd_clear();
}
