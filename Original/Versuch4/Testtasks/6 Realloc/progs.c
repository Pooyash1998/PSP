...
MemAddr volatile addrI = 0;
MemAddr volatile addrE = 0;
...
void runTest(Heap* heap) {
    ...
    // Test if the ownership check is correctly implemented.
    // Since we have two concurrently running test processes,
    // we first allocate some memory for the other process, which then tries to reallocate this memory
    if (heap == intHeap && IS_TESTING(extHeap)) {
        addrE = os_malloc(extHeap, 42);
    } else if (heap == extHeap && IS_TESTING(intHeap)) {
        addrI = os_malloc(intHeap, 42);
    }
    
    if (IS_TESTING(heap)) {
        // Try to reallocate memory of another process
        test('9', PSTR("Owner"), heap);
        
        // Wait until the memory has been allocated
        while ((heap == intHeap ? addrI : addrE) == 0);
